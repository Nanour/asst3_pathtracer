...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double cameraSpacex = screenW * (x - 0.5) / screenDist;
  double cameraSpacey = screenH * (y - 0.5) / screenDist; 
  Vector3D rayDirection = c2w * Vector3D(cameraSpacex, cameraSpacey, -1);
